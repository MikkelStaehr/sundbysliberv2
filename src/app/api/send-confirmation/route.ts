...
async function handleConfirmation(token: string, req: NextRequest) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    return handleConfirmation(token, req);
  } catch (err) {
    console.error("Fejl ved send-confirmation (POST):", err);
    return NextResponse.redirect(new URL("/", req.url));
  }
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("Fejl ved send-confirmation (GET):", err);
    return NextResponse.redirect(new URL("/", req.url));
  }
}
